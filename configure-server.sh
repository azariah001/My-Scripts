...
echo "Before we can get started we're going to need a few things, namely nginx letsencrypt and ufw."
...
if [ -e /etc/nginx/sites-available/$1 ]; then
  echo "Backing up existing nginx config like a boss."
...
echo "Writing temporary nginx config file for Let's Encrypt Setup $1"
echo "
server {
        listen 80;
        listen [::]:80;
...
if [ ! -e /etc/nginx/sites-enabled/$1 ]; then
  echo "Enabled this site is!"
  ln -s /etc/nginx/sites-available/$1 /etc/nginx/sites-enabled/$1
else
  echo "Check the backup config, you should! Already enabled this site was!"
fi

echo "Restarting nginx with the new configuration for letsencrypt."
service nginx restart

## Setup let's encrypt certs
echo "We're now going to setup the letsencrypt certs, if this is the first time letsencrypt has been run on this machine it will prompt you for some input please be sure to fill out your details correctly as they will be needed should you ever need to recover or revoke ssl certs as part of damage mitigation."

letsencrypt certonly -a webroot --webroot-path=/var/www/$1 -d $1 -d $1

echo "Should you need to backup your ssl certs for this site, or access them for any other reasons, they are located under /etc/letsencrypt/live/$1 You should see the files listed below."
ls -l /etc/letsencrypt/live/$1

mkdir /etc/nginx/snippets
echo "Creating site specific ssl config file for easy inclusion in main configuration file. If you would like to edit any of the following config files manually they are located under /etc/nginx/snippets/"
echo "
...
if [ ! -e /etc/ssl/certs/dhparam.pem ]; then
  echo "We are now going to generate a strong Diffie-Hellman group to strengthen the ssl encryption, this will take some time depending on your systems configuration."
  openssl dhparam -out /etc/ssl/certs/dhparam.pem 2048
fi
if [ ! -e /etc/nginx/snippets/ssl-params.conf ]; then
  echo "We're also creating a config file for the Diffie-Hellman group, this is reusable so you won't see messages relating to Diffie-Hellman the next time you create a site on this server. Please note that there is a HSTS option that you may wish to configure in this file, ssl-params.conf"
  echo "
...
echo "Writing final nginx config file, this includes a redirect from http to https and configuration of the .well-known/ directory for the letsencrypt's automated renewal service which we'll setup in a minute and a proxy for forwarding traffic to a node.js instance running on a non-standard port."
echo "
server {
    listen 80;
    listen [::]:80;
...
echo "Would you like to do a first time setup of UFW? [y/n]"
read prompt
if [prompt == 'y'] then;

  echo "Setting UFW rules to allow for inbound nginx traffic, we're also making sure that OpenSSH is allowed to ensure we don't lock you out of your server because 'Awkward that would be mrrrh?'"
